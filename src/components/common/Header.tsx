...
export const Header: React.FC<HeaderProps> = ({ activeTab, setActiveTab }) => {
  ...
  const getRoleLabel = (role: Role | string) => {
    switch (role) {
      ...
      case 'consumer':
        return 'Konsumen Pembeli';
      default:
        return role ? String(role) : 'Pengguna';
    }
  };

  return (
    <>
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors pt-6 sm:pt-8 pb-3.5 shadow-xs">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          <div className="flex items-center justify-between min-h-14 gap-2 sm:gap-4">
            {/* Brand Logo & Name */}
            ...
            {/* Center Controls: Store Badge & Role Badge */}
            <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
              {/* Active Store Badge (Static, no store list dropdown) */}
              <div className="flex items-center gap-1.5 py-1.5 px-2.5 rounded-lg border border-slate-200 dark:border-slate-750 bg-slate-50 dark:bg-slate-800/80 text-[11px] sm:text-xs font-semibold text-slate-700 dark:text-slate-200 max-w-[130px] sm:max-w-[200px]">
                <Store className="w-3.5 h-3.5 text-sky-500 shrink-0" />
                <span className="truncate">{store?.name || 'Toko Optik'}</span>
              </div>
              ...
              <div className="relative">
                ...
                {isUserOpen && (
                  <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 p-2.5 z-50">
                    {/* Only Owner can switch between employees */}
                    {isOwner && (
                      <>
                            ...
                            >
                              <div>
                                ...
                                <div className="text-[10px] text-slate-400">
                                  Peran: {Array.isArray(emp.roles) ? emp.roles.map((r) => getRoleLabel(r)).join(' • ') : getRoleLabel((emp as any)?.role || 'pelayan')}
                                </div>
                              </div>
                              {emp.id === currentUser?.id && (
